...
void setup() 
{
    ...
    dist.addIR(DistL,350); //add all five distance sensors
    dist.addIR(DistFL,150);
    dist.addIR(DistF,150);
    dist.addIR(DistFR,150);
    dist.addIR(DistR,350);
...
void loop() 
{
	...
	if (botLoc[0] == '1' && botLoc[4] == '1') //left AND right
	    mode = 0; //act as if saw nothing
        else if (botLoc[0] == '1') //left
            mode = 4;
    ...
    if (mode == 0) //sees nothing
    {
        ...
        {
            if (was_left)
                motors.move(-pivot_speed,pivot_speed);
            else
                motors.move(pivot_speed,-pivot_speed);
        }
    }
